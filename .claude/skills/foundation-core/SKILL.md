...
description: >
  Provides 에이전트 시스템 foundational principles including TRUST 5 quality framework,
  ...
  agent delegation patterns, or orchestration rules.
  Do NOT use for context and token management (use foundation-context instead)
  or strategic analysis (use foundation-philosopher instead).
license: Apache-2.0
...
Foundational principles and architectural patterns that power 에이전트 시스템's AI-driven development workflow.
...
Phase 1 SPEC (/plan): workflow-spec generates EARS format. Output is .sudapapa/specs/SPEC-XXX/spec.md. Execute /clear to save 45-50K tokens.

Phase 2 DDD (/run): ANALYZE for requirements, PRESERVE for existing behavior, IMPROVE for enhancement. Validate with at least 85% coverage.

Phase 3 Docs (/sync): API documentation, architecture diagrams, project reports.
...
Core Principle: 오케스트레이터는 위임해야 함 all work through Task() to specialized agents. Direct execution bypasses specialization, quality gates, and token optimization. Proper delegation improves task success rate by 40 percent and enables parallel execution.
...
Phase Separation: Execute /clear between phases, after /plan to save 45-50K, when context exceeds 150K, after 50+ messages.
...
Commands: /plan for SPEC-First Phase 1, /run for DDD Phase 2, /sync for Documentation Phase 3, /feedback for continuous improvement, /clear for token management.
