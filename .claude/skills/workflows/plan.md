...
- Implements Steps 1-2 of 4단계 워크플로우 (Intent Understanding, Plan Creation)
- Steps 3-4 are handled by /run and /sync respectively
...
- .sudapapa/config/config.yaml (git strategy, language settings)
...
- .sudapapa/project/product.md (product context)
- .sudapapa/project/structure.md (architecture context)
- .sudapapa/project/tech.md (technology context)
- .sudapapa/specs/ directory listing (existing SPECs for deduplication)

Pre-execution commands: git status, git branch, git log, git diff, find .sudapapa/specs.
...
- Locate existing SPEC documents in .sudapapa/specs/
...
- Check for duplicate SPECs in .sudapapa/specs/
...
- Reports route to .sudapapa/reports/, Documentation to .sudapapa/docs/
...
- ID Uniqueness: Search .sudapapa/specs/ to confirm no duplicates exist
...
- .sudapapa/specs/SPEC-{ID}/spec.md
...
- Stage SPEC files: git add .sudapapa/specs/SPEC-{ID}/
...
- Start Implementation (execute /run SPEC-{ID})
- Modify Plan
...
- Directory naming follows .sudapapa/specs/SPEC-{ID}/ format
...
Version: 2.0.0
Updated: 2026-02-07
Source: Extracted from .claude/commands/1-plan.md v5.1.0. Added team mode support and --team flag.
