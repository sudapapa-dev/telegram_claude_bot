...
- Implements Step 3 of 4단계 워크플로우 (Task Execution)
- Receives SPEC documents created by /plan
- Hands off to /sync for documentation and PR

## Input

- $ARGUMENTS: SPEC-ID to implement (e.g., SPEC-AUTH-001)
- Resume: Re-running /run SPEC-XXX resumes from last successful phase checkpoint
...
- .sudapapa/config/config.yaml (git strategy, automation settings)
- .sudapapa/config/sections/quality.yaml (coverage targets, TRUST 5 settings)
- .sudapapa/config/sections/git-strategy.yaml (auto_branch, branch creation policy)
- .sudapapa/config/sections/language.yaml (git_commit_messages setting)
- .sudapapa/specs/SPEC-{ID}/ directory (spec.md, plan.md, acceptance.md)
...
Before Phase 2, determine the development methodology by reading `.sudapapa/config/sections/quality.yaml`:
...
This divergence data is consumed by /sync for SPEC document updates and project document synchronization.
...
- Sync Documentation (recommended): Execute /sync to synchronize docs and create PR
- Implement Another Feature: Return to /plan for additional SPEC
...
- Phase 2 to /sync: Implementation divergence report enables accurate SPEC and project document updates
...
Version: 2.1.0
Updated: 2026-02-13
Source: Extracted from .claude/commands/2-run.md v5.0.0. Added implementation divergence tracking, development_mode routing (ddd/tdd/hybrid), team mode support, LSP quality gates, extended quality checks (code complexity, dead code, side effects, code reuse), plan proportionality validation, and post-implementation review loop.
