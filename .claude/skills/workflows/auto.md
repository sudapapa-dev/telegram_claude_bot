...
metadata:
  ...
  status: "active"
  updated: "2026-02-07"
  tags: "autonomous, pipeline, plan-run-sync, default"
...
# Extension: Triggers
triggers:
  keywords: ["auto", "autonomous", "pipeline", "build", "implement", "create"]
  agents: ["orchestrator"]
  phases: ["plan", "run", "sync"]
---

# Workflow: 자율 개발 오케스트레이션

Purpose: Full autonomous workflow. 사용자가 목표를 제공하면 시스템이 자율적으로 실행 plan -> run -> sync pipeline. This is the default workflow when no subcommand is specified.
...
[HARD] Before Phase 2 implementation, ALWAYS check `.sudapapa/config/sections/quality.yaml`:
...
- Output: EARS-format SPEC document at .sudapapa/specs/SPEC-XXX/spec.md
...
[HARD] Methodology selection based on `.sudapapa/config/sections/quality.yaml`:
...
- Conditionally update project documents (.sudapapa/project/) when structural changes detected
