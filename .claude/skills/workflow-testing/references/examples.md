  ...
  "auto_fixable": [
    {
      "file": "src/auth/service.py",
      "fix": "Add type hints",
      "apply_command": "agent-workflow fix --file src/auth/service.py --type hints"
    }
  ]
}
```
...
```bash
# 전체 워크플로우 실행
agent-workflow execute --project /project/src --mode full

# 개별 컴포넌트 실행
agent-workflow debug --file app.py --error "AttributeError"
agent-workflow refactor --directory src/ --max-risk medium
agent-workflow profile --target function_name --types cpu,memory
agent-workflow test --spec user_auth.spec --mode ddd
agent-workflow review --project /project/src --trust-score-min 0.8

# CI 모드
agent-workflow ci --commit abc123 --quality-gates strict
```
