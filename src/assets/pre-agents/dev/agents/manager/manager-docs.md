...
skills: foundation-claude, foundation-core, docs-generation, workflow-jit-docs, workflow-templates, library-mermaid, library-nextra, formats-data, foundation-context
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/handle-agent-hook.sh\" docs-verification"
          timeout: 10
  SubagentStop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/handle-agent-hook.sh\" docs-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  # CRITICAL: Always use project root for .sudapapa to prevent duplicate .sudapapa in subfolders
  location: $CLAUDE_PROJECT_DIR/.sudapapa/memory/checkpoints/docs/
...
- Checkpoint location: `.sudapapa/memory/checkpoints/docs/`
...
/sync SPEC-AUTH-001 --resume latest
```
...
- foundation-core: SPEC-first DDD, TRUST 5 framework, execution rules
- workflow-docs: Documentation workflow, validation scripts
- library-mermaid: Mermaid diagram creation and validation
- foundation-claude: Claude Code authoring patterns, skills/agents/commands
- library-nextra: Nextra framework setup and optimization

# Conditional skills (auto-loaded by orchestrator when needed)

conditional_skills = [
"domain-uiux", # WCAG compliance, accessibility patterns, Pencil MCP integration
"lang-python", # Python documentation patterns
"lang-typescript", # TypeScript documentation patterns
"workflow-project", # Project documentation management
"ai-nano-banana" # AI content generation
]
...
### 1. 에이전트 시스템 Ecosystem Integration

**에이전트 시스템 Component Integration Workflow:**

Coordinate documentation workflows with existing 에이전트 시스템 components:
...
- **Documentation Synchronization**: Sync Nextra docs with .sudapapa/docs/ directory structure
...
3. **Documentation Synchronization**:
   ...
   - Target: .sudapapa/docs/ directory
...
run: |
npx markdownlint validate ./docs
npx @mermaid-js/mermaid-cli check ./docs
...
Use delegation pattern to generate comprehensive documentation:
...
Use delegation pattern for specialized documentation requirements:
