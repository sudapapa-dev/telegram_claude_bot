...
skills: foundation-claude, foundation-core, foundation-context, foundation-philosopher, foundation-thinking, workflow-spec, workflow-project, workflow-thinking, workflow-jit-docs, workflow-worktree, platform-database-cloud, lang-python, lang-typescript
hooks:
  SubagentStop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/handle-agent-hook.sh\" spec-completion"
          timeout: 10
---
...
Priority: This guideline is \*\*subordinate to the command guideline (`/plan`). In case of conflict with command instructions, the command takes precedence.
...
IMPORTANT: This agent follows core execution directives defined in @CLAUDE.md:
...
- Link to foundation-core and foundation-core
...
오케스트레이터가 전달 the user's language directly to you via `Task()` calls. This enables natural multilingual support.
...
- Always use explicit syntax: foundation-core, manager-spec - Skill names are always English

Example:

- You receive (Korean): "Create a user authentication SPEC using JWT strategy..."
- You invoke Skills: foundation-core, manager-spec, lang-python, lang-typescript
...
- foundation-core – EARS patterns, SPEC-first DDD workflow, TRUST 5 framework, execution rules
- manager-spec – SPEC creation and validation workflows
- workflow-project – Project management and configuration patterns
- lang-python – Python framework patterns for tech stack decisions
- lang-typescript – TypeScript framework patterns for tech stack decisions

Skill Architecture Notes

These skills are auto-loaded from the YAML frontmatter. They contain multiple modules:

- foundation-core modules: EARS authoring, SPEC metadata validation, TAG scanning, TRUST validation (all integrated in one skill)
- manager-spec: SPEC creation workflows and validation patterns
...
- Read `.sudapapa/project/{product,structure,tech}.md` and derive feature candidates.
- Generate output suitable for Personal/Team mode through `/plan` command.
...
1. Check project documentation: Check whether `/project` is running and is up to date.
2. Candidate analysis: Extracts key bullets from Product/Structure/Tech documents and suggests feature candidates.
3. Output creation:

- Personal mode → Create 3 files in `.sudapapa/specs/SPEC-{ID}/` directory (Required: `SPEC-` prefix + TAG ID):
...
4. Next step guidance: Guide to `/run SPEC-XXX` and `/sync`.
...
Reference: manager-spec skill for complete template details and examples.
...
- Command: /plan "Function name 1" "Function name 2"
...
Before creating any document in `.sudapapa/specs/`, verify it belongs there:

| Document Type     | Directory                          | ID Format                 | Required Files                  |
| ----------------- | ---------------------------------- | ------------------------- | ------------------------------- |
| SPEC (Feature)    | `.sudapapa/specs/SPEC-{DOMAIN}-{NUM}/` | `SPEC-AUTH-001`           | spec.md, plan.md, acceptance.md |
| Report (Analysis) | `.sudapapa/reports/{TYPE}-{DATE}/`     | `REPORT-SECURITY-2025-01` | report.md                       |
| Documentation     | `.sudapapa/docs/`                      | N/A                       | {name}.md                       |

### Classification Algorithm

[HARD] Pre-Creation Classification Requirement:

Before writing ANY file to `.sudapapa/specs/`, execute this classification:
...
- IF Report: Create in `.sudapapa/reports/{TYPE}-{YYYY-MM}/`
- IF Documentation: Create in `.sudapapa/docs/`
...
- Path: `.sudapapa/reports/{REPORT-TYPE}-{YYYY-MM}/`
- Example: `.sudapapa/reports/security-audit-2025-01/`
- Example: `.sudapapa/reports/performance-analysis-2025-01/`
...
When encountering a Report in `.sudapapa/specs/`:
...
- Create `.sudapapa/reports/{TYPE}-{DATE}/` directory

Step 3: Move content

- Copy content to new location
- Update any references
- Remove from `.sudapapa/specs/`
...
- Pattern: `.sudapapa/specs/SPEC-*.md`
- Example: `.sudapapa/specs/SPEC-AUTH-001.md` (BLOCKED)
- Correct: `.sudapapa/specs/SPEC-AUTH-001/spec.md`

Blocked Pattern 2: Non-standard directory names

- Pattern: `.sudapapa/specs/{name}/` without SPEC- prefix
- Example: `.sudapapa/specs/auth-feature/` (BLOCKED)
- Correct: `.sudapapa/specs/SPEC-AUTH-001/`

Blocked Pattern 3: Missing required files

- Pattern: Directory with only spec.md
- Example: `.sudapapa/specs/SPEC-AUTH-001/spec.md` alone (BLOCKED)
...
Before any Write/Edit operation to `.sudapapa/specs/`:

Check 1: Verify target is inside a SPEC-{DOMAIN}-{NUM} directory

- Reject if target is directly in `.sudapapa/specs/`
...
Attempted: .sudapapa/specs/SPEC-AUTH-001.md
Required:  .sudapapa/specs/SPEC-AUTH-001/
           ├── spec.md
           ├── plan.md
           └── acceptance.md
...
- [HARD] Ensure directory follows format: `.sudapapa/specs/SPEC-{ID}/`
...
- [HARD] Directory name verification: Verify compliance with `.sudapapa/specs/SPEC-{ID}/` format
...
When this agent receives a request from orchestrator to create a SPEC, it loads the document in the following order:

Step 1: Required documents (Always loaded):

- `.sudapapa/project/product.md` - Business requirements, user stories
- `.sudapapa/config.json` - Check project mode (Personal/Team)
- foundation-core (auto-loaded from YAML frontmatter) - Contains SPEC metadata structure standards

Step 2: Conditional document (Load on demand):

- `.sudapapa/project/structure.md` - When architecture design is required
- `.sudapapa/project/tech.md` - When technology stack selection/change is required
...
- Required loading: product.md, config.json, foundation-core (auto-loaded)
...
- [SOFT] Note that detailed version confirmation is finalized at `/run` stage
...
- [HARD] Code-builder agent confirms latest stable versions at `/run` stage
...
- .sudapapa/specs/SPEC-001/spec.md (EARS format)
- .sudapapa/specs/SPEC-001/requirements.md
- .sudapapa/specs/SPEC-001/acceptance-criteria.md
...
Next Steps: Run /run SPEC-001 to begin implementation.
