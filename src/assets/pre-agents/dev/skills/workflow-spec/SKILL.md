...
description: >
  ...
  and Plan-Run-Sync integration for 에이전트 시스템 development methodology.
  Use when creating SPEC documents, writing EARS requirements, defining acceptance
  criteria, planning features, or orchestrating the /plan phase.
  Do NOT use for implementation (use workflow-ddd instead)
  or documentation generation (use workflow-project instead).
license: Apache-2.0
...
metadata:
  ...
  updated: "2026-01-08"
  modularized: "true"
  tags: "workflow, spec, ears, requirements, adk, planning"
  author: "에이전트 시스템 Team"
  context: "fork"
  agent: "Plan"
...
- Create new SPEC: /plan "user authentication system"
- Create parallel SPECs with Worktrees: /plan "login feature" "signup feature" --worktree
- Create SPEC with new branch: /plan "payment processing" --branch
- Update existing SPEC: /plan SPEC-001 "add OAuth support"
...
Constitution defines the project DNA that all SPECs must respect. Before creating any SPEC, verify alignment with project constitution defined in `.sudapapa/project/tech.md`.
...
- SPEC document creation in .sudapapa/specs/ directory
...
- workflow-testing skill reference for test patterns
...
- Command /plan "login feature" "signup feature" --worktree creates multiple SPECs
...
- .sudapapa/specs/SPEC-{ID}/: SPEC document directory containing 3 required files
...
- .sudapapa/memory/: Session state files (last-session-state.json)
- .sudapapa/docs/: Generated documentation (api-documentation.md)
...
- foundation-core: SPEC-First DDD methodology and TRUST 5 framework
- workflow-testing: DDD implementation and test automation
- workflow-project: Project initialization and configuration
- workflow-worktree: Git Worktree management for parallel development
...
- Step 1 PLAN: /plan "user authentication system"
- Step 2 RUN: /run SPEC-001
- Step 3 SYNC: /sync SPEC-001

Parallel Workflow:

- Create multiple SPECs: /plan "backend API" "frontend UI" "database schema" --worktree
- Session 1: /run SPEC-001 (backend API)
- Session 2: /run SPEC-002 (frontend UI)
- Session 3: /run SPEC-003 (database schema)
...
- SPEC document persists in .sudapapa/specs/ directory
- Session memory in .sudapapa/memory/ for cross-session context
...
### What Belongs in .sudapapa/specs/

The `.sudapapa/specs/` directory is EXCLUSIVELY for SPEC documents that define features to be implemented.
...
### What Does NOT Belong in .sudapapa/specs/

| Document Type         | Why Not SPEC                  | Correct Location                          |
| --------------------- | ----------------------------- | ----------------------------------------- |
| Security Audit        | Analyzes existing code        | `.sudapapa/reports/security-audit-{DATE}/`    |
| Performance Report    | Documents current metrics     | `.sudapapa/reports/performance-{DATE}/`       |
| Dependency Analysis   | Reviews existing dependencies | `.sudapapa/reports/dependency-review-{DATE}/` |
| Architecture Overview | Documents current state       | `.sudapapa/docs/architecture.md`              |
| API Reference         | Documents existing APIs       | `.sudapapa/docs/api-reference.md`             |
| Meeting Notes         | Records decisions made        | `.sudapapa/reports/meeting-{DATE}/`           |
| Retrospective         | Analyzes past work            | `.sudapapa/reports/retro-{DATE}/`             |

### Exclusion Rules

[HARD] Report vs SPEC Distinction:

Reports analyze what EXISTS → `.sudapapa/reports/`
SPECs define what will be BUILT → `.sudapapa/specs/`

[HARD] Documentation vs SPEC Distinction:

Documentation explains HOW TO USE → `.sudapapa/docs/`
SPECs define WHAT TO BUILD → `.sudapapa/specs/`
...
Problem: `.sudapapa/specs/SPEC-AUTH-001.md` exists as single file

Solution Steps:

1. Create directory: `mkdir -p .sudapapa/specs/SPEC-AUTH-001/`
2. Move content: `mv .sudapapa/specs/SPEC-AUTH-001.md .sudapapa/specs/SPEC-AUTH-001/spec.md`
...
find .sudapapa/specs -maxdepth 1 -name "SPEC-*.md" -type f
```
...
1. Find next available number:
   ```bash
   ls -d .sudapapa/specs/SPEC-*-[0-9][0-9][0-9] 2>/dev/null | sort -t- -k3 -n | tail -1
   ```
2. Assign number: `SPEC-REDESIGN` → `SPEC-REDESIGN-001`
3. Rename directory:
   ```bash
   mv .sudapapa/specs/SPEC-REDESIGN .sudapapa/specs/SPEC-REDESIGN-001
   ```
...
Problem: Analysis/audit document in `.sudapapa/specs/`
...
2. Create reports directory:
   ```bash
   mkdir -p .sudapapa/reports/security-audit-2025-01/
   ```
3. Move content:
   ```bash
   mv .sudapapa/specs/SPEC-SECURITY-AUDIT/* .sudapapa/reports/security-audit-2025-01/
   rmdir .sudapapa/specs/SPEC-SECURITY-AUDIT
   ```
...
1. Compare creation dates:
   ```bash
   ls -la .sudapapa/specs/ | grep SPEC-AUTH-001
   ```
2. Determine which is canonical (usually older one)
3. Renumber newer one to next available:
   ```bash
   mv .sudapapa/specs/SPEC-AUTH-001-duplicate .sudapapa/specs/SPEC-AUTH-002
   ```
...
for dir in .sudapapa/specs/SPEC-*/; do
  if [ -d "$dir" ]; then
    missing=""
...
ls -d .sudapapa/specs/SPEC-*/ 2>/dev/null | grep -v -E 'SPEC-[A-Z]+-[0-9]{3}'

# Check 4: Potential reports in specs
echo -e "\n[Check 4] Potential reports in specs (check manually):"
grep -l -r "findings\|recommendations\|audit\|analysis" .sudapapa/specs/*/spec.md 2>/dev/null
