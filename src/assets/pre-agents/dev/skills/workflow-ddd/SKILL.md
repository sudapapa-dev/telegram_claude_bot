...
description: >
  ...
  Do NOT use for writing new tests (use workflow-testing instead)
...
metadata:
  ...
  context: "fork"
  agent: "manager-ddd"
  related-skills: "tool-ast-grep, workflow-testing, foundation-quality"
---
...
[NOTE] This workflow is selected based on `.sudapapa/config/sections/quality.yaml`:
...
- `development_mode: hybrid` + new package/module → Use TDD instead (workflow-tdd)
...
When executing DDD through /run in DDD mode:
...
Load tool-ast-grep for detailed pattern syntax and rule creation.
