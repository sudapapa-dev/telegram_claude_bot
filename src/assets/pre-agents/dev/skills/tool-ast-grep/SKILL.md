...
AST-Grep is integrated into 에이전트 시스템 through the Tool Registry as AST_ANALYZER type in internal/hook/registry.go, PostToolUse Hook for automatic security scanning after Write/Edit operations, and Permissions with Bash(sg:*) and Bash(ast-grep:*) auto-allowed.

### Running Scans

To scan with 에이전트 시스템 rules, execute sg scan with config pointing to .claude/skills/tool-ast-grep/rules/sgconfig.yml.
