...
description: >
  ...
  automatic branch registration, and seamless 에이전트 시스템 integration.
...
Git worktree management system for parallel SPEC development with isolated workspaces, automatic registration, and seamless 에이전트 시스템 integration.
...
- Seamless Integration: Works with /plan, /run, /sync workflow
...
5. Integration Layer - 에이전트 시스템 workflow integration
...
The worktree system creates a dedicated directory structure inside the project's .sudapapa directory. At the worktree root ({repo}/.sudapapa/worktrees/{ProjectName}/), you will find the central registry JSON file and individual directories for each SPEC. Each SPEC directory contains a .git file for worktree metadata and a complete copy of all project files.
...
During the Plan Phase using /plan, the SPEC is created and the worktree new command sets up automatic worktree isolation.
...
During the Sync Phase using /sync, the worktree sync command ensures clean integration with conflict resolution support.
...
First, create a worktree for SPEC-001 with a description like "User Authentication" and switch to that directory. Then run /run SPEC-001 to develop in isolation. Next, navigate back to the main repository and create another worktree for SPEC-002 with description "Payment Integration". Switch to that worktree and run /run SPEC-002 for parallel development. When needed, switch between worktrees and continue development. Finally, sync both worktrees when ready for integration.

Detailed Reference: Refer to Parallel Development Module at modules/parallel-development.md

---

### 4. Integration Patterns - 에이전트 시스템 Workflow Integration

Purpose: Seamless integration with 에이전트 시스템 Plan-Run-Sync workflow.

Integration Points:

During Plan Phase Integration with /plan, after SPEC creation, create the worktree using the new command with the SPEC ID. The output provides guidance for switching to the worktree using either the switch command or the shell eval pattern with the go command.

During Development Phase with /run, worktree isolation provides a clean development environment with independent Git state preventing conflicts and automatic registry tracking.

During Sync Phase with /sync, before PR creation run the sync command for the SPEC. After PR merge, run the clean command with the merged-only flag to remove completed worktrees.
...
에이전트 시스템 설정은 worktree 옵션을 지원함 including auto_create for automatic worktree creation, auto_sync for automatic synchronization, cleanup_merged for automatic cleanup of merged branches, and worktree_root for specifying the worktree directory location with project name substitution.
...
Commands:
- /plan - SPEC creation with automatic worktree setup
- /run - Development in isolated worktree environment
- /sync - Integration with automatic worktree sync
- /feedback - Worktree workflow improvements

Skills:
- foundation-core - Parallel development patterns
- workflow-project - Project management integration
- workflow-spec - SPEC-driven development
- git-strategy - Git workflow optimization
...
For new SPEC development, use the worktree isolation pattern with auto-setup. The primary approach is worktree isolation and the supporting pattern is integration with /plan.
...
- Integration Patterns: Refer to modules/integration-patterns.md for 에이전트 시스템 integration
