...
- Integration Issues: 에이전트 워크플로우 integration and command coordination

Quick Diagnostics:
- Check worktree status: worktree-cli status --all
- Verify registry integrity: worktree-cli registry validate
...
Prevention:
- Always use worktree-cli remove instead of manual directory deletion
...
Symptoms:
- worktree-cli commands fail with JSON parsing errors
...
- Validate the rebuilt registry with worktree-cli status --all

### Orphaned Registry Entries

Symptoms:
- Registry lists worktrees that no longer exist on disk
- worktree-cli status shows worktrees as missing
...
Resolution Steps:
1. Run worktree-cli status --all to identify orphaned entries
2. For each orphaned entry, confirm the directory truly does not exist
3. Remove orphaned entries from registry using worktree-cli registry prune
...
- Use worktree-cli commands for standard operations
...
Symptoms:
- /plan does not create expected worktree
- /run cannot find worktree for SPEC
- Workflow commands operate on wrong worktree

Root Causes:
- SPEC ID not matching worktree ID format
- Worktree를 에이전트 시스템 통합 없이 수동으로 생성한 경우
- Configuration mismatch between the system and worktree settings
- Working directory context not in expected location

Resolution Steps:
1. Verify SPEC ID format matches expected worktree naming convention
2. Ensure worktree was created through 에이전트 워크플로우 or properly registered
3. Check 에이전트 시스템 설정 (worktree 항목) integration settings
...
Status Commands:
- worktree-cli status --all: Shows all worktrees with sync status
- git worktree list: Native Git worktree listing
- worktree-cli status SPEC-ID --detailed: Detailed status for specific worktree

Registry Commands:
- worktree-cli registry validate: Checks registry integrity
- worktree-cli registry prune: Removes orphaned entries
- worktree-cli registry export: Exports registry for backup
...
Cleanup Commands:
- worktree-cli clean --dry-run: Preview cleanup without changes
- worktree-cli clean --merged-only: Clean only merged worktrees
- git worktree prune: Remove stale Git worktree metadata

Recovery Commands:
- worktree-cli registry rebuild: Reconstruct registry from directories
- worktree-cli remove SPEC-ID --keep-branch: Remove worktree, preserve branch
...
Weekly Tasks:
- Run worktree-cli status --all to check for issues
...
Creation:
- Always use worktree-cli new instead of manual Git commands
...
Removal:
- Use worktree-cli remove instead of manual deletion
