# 에이전트 시스템 Integration Module

Purpose: Detailed integration patterns for worktree-cli with 에이전트 시스템 Plan-Run-Sync workflow including plan phase automation, DDD integration, and cleanup workflows.
...
에이전트 시스템 Integration Points:
- /plan: Automatic worktree creation after SPEC generation
- /run: DDD execution in isolated worktree environment
- /sync: Worktree sync with documentation updates
- Cleanup: Automatic removal of merged worktrees

---

## Plan Phase Integration (/plan)
...
- Default: {repo}/.sudapapa/worktrees/{project-name}/SPEC-{id}
- Configurable via worktree_root setting

Creation Workflow:
1. SPEC is created with /plan
...
## Development Phase Integration (/run)
...
- Looks for .sudapapa/worktrees directory in path hierarchy
...
Worktree settings in .sudapapa/config/config.yaml:
...
- worktree_root: Base directory for worktrees (default: {repo}/.sudapapa/worktrees)
...
Version: 1.0.0
Last Updated: 2026-01-06
Module: 에이전트 시스템 workflow integration patterns for Plan-Run-Sync phases
