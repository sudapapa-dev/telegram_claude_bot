...
Purpose: Overview of integration patterns for worktree-cli with 에이전트 시스템 workflow, development tools, and external systems.
...
Integration Points:
- 에이전트 시스템 Workflow: Seamless integration with /plan, /run, /sync
...
The basic integration workflow follows three phases. During the Plan Phase, the /plan command auto-creates a worktree for the SPEC. During the Development Phase, use the worktree-cli go command to navigate to the isolated environment and run /run for DDD implementation. During the Sync Phase, use worktree-cli sync to update the worktree and /sync for documentation synchronization.
...
에이전트 시스템 Integration: Refer to agent-integration.md
- Plan Phase integration with /plan
- Development Phase integration with /run
- Sync Phase integration with /sync
...
### 에이전트 시스템 Workflow Integration

The worktree system integrates deeply with the Plan-Run-Sync workflow:
...
Detailed Reference: Refer to agent-integration.md for complete workflow patterns
...
For automatic SPEC worktree creation, integrate with /plan by configuring auto-creation in worktree settings. Refer to agent-integration.md for implementation details.
...
For team collaboration with shared registries, configure team registry settings with developer-specific worktree prefixes. Refer to agent-integration.md for coordination patterns.

---

## Sub-Module References

에이전트 시스템 Integration (agent-integration.md):
- Complete /plan integration patterns
- DDD-aware /run integration
- Sync Phase automation with /sync
