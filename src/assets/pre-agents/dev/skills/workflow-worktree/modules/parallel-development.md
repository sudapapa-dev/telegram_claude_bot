...
To set up parallel development, create worktrees for each SPEC using worktree-cli new with the SPEC ID and description. For parallel execution, navigate to each worktree using worktree-cli go and run /run in separate terminals or background processes. For integration, use worktree-cli sync --all to synchronize all worktrees, then clean up merged worktrees with worktree-cli clean --merged-only.
...
Worktree Root ({repo}/.sudapapa/worktrees/{project-name}/):
