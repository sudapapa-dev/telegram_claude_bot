...
Purpose: External resources, documentation, and additional learning materials for worktree-cli skill.
...
- Click Framework: [https://click.palletsprojects.com/](https://click.palletsprojects.com/)
 - Command-line interface framework used by worktree-cli
...
- Rich Library: [https://rich.readthedocs.io/](https://rich.readthedocs.io/)
 - Terminal output formatting used by worktree-cli
...
- foundation-core: Foundation principles and delegation patterns
 - Reference: [foundation-core modules](foundation-core/modules/)
 - Integration: Worktree creation follows foundation delegation patterns

- workflow-project: Project management and configuration
 - Reference: [workflow-project modules](workflow-project/modules/)
 - Integration: Project setup with worktree support

- foundation-claude: Claude Code execution patterns
 - Reference: [foundation-claude modules](foundation-claude/modules/)
...
1. Shallow Worktrees: For fast prototyping and testing
 ```bash
 worktree-cli new SPEC-PROTO-001 "Prototype" --shallow --depth 1
 ```

2. Selective Synchronization: Sync only essential files
 ```bash
 worktree-cli sync SPEC-001 --include "src/" --exclude "node_modules/"
 ```

3. Background Operations: Non-blocking worktree operations
 ```bash
 worktree-cli sync --all --background
 ```
...
jobs:
 ...
 - name: Setup worktree-cli
 run: |
 go install github.com/modu-ai-adk/cmd@latest
 echo "Setting up worktree environment..."

 - name: Test Worktree Operations
 run: |
 # Test worktree creation
 worktree-cli new test-spec "Test Worktree"

 # Test worktree synchronization
 worktree-cli sync test-spec

 # Test worktree cleanup
 worktree-cli remove test-spec
```
...
pipeline {
 ...
 sh '''
 go install github.com/modu-ai-adk/cmd@latest
 worktree-cli config set worktree_root $WORKSPACE/worktrees
 '''
 }
 }
 ...
 steps {
 sh '''
 worktree-cli new SPEC-AUTH-001 "Authentication Worktree"
 cd $(worktree-cli go SPEC-AUTH-001)
 npm test
 '''
 }
 }
 ...
 steps {
 sh '''
 worktree-cli new SPEC-PAY-001 "Payment Worktree"
 cd $(worktree-cli go SPEC-PAY-001)
 npm test
 '''
 }
 }
...
- Integration with 에이전트 시스템 workflow commands
...
git clone https://github.com/your-org-adk.git
...
Version: 1.0.0
Last Updated: 2025-11-29
Reference: External resources and additional learning materials for worktree-cli
