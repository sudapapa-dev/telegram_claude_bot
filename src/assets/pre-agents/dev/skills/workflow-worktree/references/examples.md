...
Purpose: Real-world usage examples and patterns for worktree-cli skill integration with 에이전트 시스템 workflow.
...
/plan "User Authentication System" --worktree

# Output:
# SPEC created: SPEC-AUTH-001
# Worktree created: .sudapapa/worktrees/에이전트 시스템/SPEC-AUTH-001
#
# Next steps:
# 1. Switch to worktree: worktree-cli switch SPEC-AUTH-001
# 2. Or use shell eval: eval $(worktree-cli go SPEC-AUTH-001)
# 3. Start development: /run SPEC-AUTH-001

# Development Phase - Implement in isolated environment
eval $(worktree-cli go SPEC-AUTH-001)
/run SPEC-AUTH-001

# Sync Phase - Synchronize and integrate
worktree-cli sync SPEC-AUTH-001
/sync SPEC-AUTH-001

# Cleanup Phase - Remove completed worktree
worktree-cli remove SPEC-AUTH-001
```
...
worktree-cli new SPEC-AUTH-001 "User Authentication"
worktree-cli new SPEC-PAY-001 "Payment Processing"
worktree-cli new SPEC-DASH-001 "Dashboard Analytics"

# Switch between worktrees
worktree-cli switch SPEC-AUTH-001
# Work on authentication...

worktree-cli switch SPEC-PAY-001
# Work on payment system...

worktree-cli switch SPEC-DASH-001
# Work on dashboard...

# Check status of all worktrees
worktree-cli status --all

# Sync all worktrees
worktree-cli sync --all

# Clean up completed worktrees
worktree-cli clean --merged-only
```
...
```bash
# /run automatically detects worktree environment
/run SPEC-AUTH-001
...
worktree-cli status SPEC-AUTH-001
worktree-cli sync SPEC-AUTH-001 --include "src/"
worktree-cli config get worktree_root
```
...
/plan "$SPEC_DESCRIPTION" --worktree --spec-id "$SPEC_ID"

# Check if worktree was created successfully
if worktree-cli list --format json | jq -r ".worktrees[\"$SPEC_ID\"]" > /dev/null; then
 echo " Worktree $SPEC_ID created successfully"

 # Phase 2: Develop
 echo " Phase 2: Switching to worktree for development..."
 cd $(worktree-cli go "$SPEC_ID")
...
WORKTREE_ROOT=".sudapapa/worktrees/$PROJECT_NAME"
...
worktree-cli config set worktree_root "$WORKTREE_ROOT"
worktree-cli config set registry_type team

echo " Team worktree configuration completed"
echo "Team members can now join with: worktree-cli join --team $TEAM_NAME"
```
...
DEBUG=true""",
 ...
 # Save template
 template_path = Path.home() / ".worktree-cli/templates" / "fullstack.json"
...
# worktree-cli new SPEC-FULL-001 "Fullstack Application" --template fullstack
```
...
class WorktreeAutomation:
 ...
 "worktree-cli", "new", spec_id, description,
 "--template", template
 ...
 result = subprocess.run([
 "worktree-cli", "list", "--status", "active", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 "worktree-cli", "status", "--all", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 'action': 'worktree-cli clean --stale --days 30'
 })
 ...
 'action': 'worktree-cli optimize --analyze'
 })
...
worktree-cli new SPEC-DEBUG-001 "Debug Test" --verbose

# Problem 2: Worktree sync conflicts
echo " Resolving sync conflicts..."

# Check sync status
worktree-cli status SPEC-CONFLICT-001

# Interactive conflict resolution
worktree-cli sync SPEC-CONFLICT-001 --interactive

# Force sync (if appropriate)
worktree-cli sync SPEC-CONFLICT-001 --force

# Problem 3: Worktree registry corruption
echo " Repairing worktree registry..."

# Backup current registry
cp .sudapapa/worktrees/PROJECT/.worktree-cli-registry.json .sudapapa/worktrees/PROJECT/.worktree-cli-registry.json.backup

# Rebuild registry from worktree directories
worktree-cli config set registry_rebuild true
worktree-cli list --rebuild-registry

# Problem 4: Permission issues
echo " Fixing permission issues..."

# Check worktree permissions
ls -la .sudapapa/worktrees/PROJECT/

# Fix permissions
chmod -R 755 .sudapapa/worktrees/PROJECT/
```
...
Version: 1.0.0
Last Updated: 2025-11-29
Examples: Real-world usage patterns for worktree-cli integration
