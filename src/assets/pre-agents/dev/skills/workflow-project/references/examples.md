...
# 프로젝트 초기화
project = WorkflowProject("./my-ecommerce-app")
...
**Explanation**: WorkflowProject는 DocumentationManager, LanguageInitializer, TemplateOptimizer 세 모듈을 통합합니다. 한국어 UI 설정, 다국어 문서 구조 생성, 템플릿 분석을 한 번에 수행합니다.
...
```json
{
  ...
  "optimization_result": {
    "status": "success",
    "backup_created": ".sudapapa/backups/templates-2025-12-06-103000/",
    "files_optimized": 12,
    "size_reduction": {
      "before": "245KB",
      "after": "198KB",
...
**Solution**: WorkflowProject로 자동 생성
...
```bash
# 프로젝트 초기화
python -m workflow_project.cli init \
    --language ko \
    --domains backend,frontend

# SPEC에서 문서 생성
python -m workflow_project.cli generate-docs \
    --spec-file .sudapapa/specs/SPEC-001.json

# 템플릿 최적화
python -m workflow_project.cli optimize-templates \
    --backup \
    --dry-run  # 미리보기

# 문서 내보내기
python -m workflow_project.cli export-docs \
    --format html \
    --language ko \
    --output ./dist/docs

# 프로젝트 상태 확인
python -m workflow_project.cli status
```
