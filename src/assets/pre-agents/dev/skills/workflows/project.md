...
description: >
  ...
  Creates product.md, structure.md, and tech.md in .sudapapa/project/ directory.
...
Purpose: Generate project documentation through smart questions and codebase analysis. Creates product.md, structure.md, and tech.md in .sudapapa/project/ directory.
...
- Existing Project (Recommended): Your codebase will be automatically analyzed to generate accurate documentation. 시스템이 스캔 your files, architecture, and dependencies to create product.md, structure.md, and tech.md.
...
- New Project (Recommended): 시스템이 안내 you through a short interview to understand your project goals, technology choices, and key features. This creates the foundation documents for all future development.
- Existing Project: Choose this if your code exists elsewhere and you want to 시스템이 분석할 경로를 지정 it.
...
- Fresh Documentation: When .sudapapa/project/ is empty, generate all three files
...
- Proceed with documentation generation (Recommended): 시스템이 생성 product.md, structure.md, and tech.md based on the analysis above. You can review and edit the documents afterwards.
...
- Output Directory: .sudapapa/project/
...
Goal: Automatically set the `development_mode` in `.sudapapa/config/sections/quality.yaml` based on the project analysis results from Phase 0 and Phase 1.
...
Implementation:
- Read current `.sudapapa/config/sections/quality.yaml`
...
- Location: .sudapapa/project/
- Status: Success or partial completion

Next Steps (AskUserQuestion):

- Write SPEC (Recommended): Execute /plan to define your first feature specification. This is the natural next step after project setup - it creates a detailed plan for what you want to build.
...
- Phase 0-2: 오케스트레이터 (AskUserQuestion for all user interaction)
...
- Phase 3.7: 오케스트레이터 (automatic development_mode configuration, no user interaction)
