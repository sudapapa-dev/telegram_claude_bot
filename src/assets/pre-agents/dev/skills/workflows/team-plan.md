...
- Triggered by: /plan --team OR auto-detected complexity >= threshold

## Phase 0: Team Setup

1. Read configuration:
   - .sudapapa/config/sections/workflow.yaml for team settings
   - .sudapapa/config/sections/quality.yaml for development mode

2. Create team:
   ```
   TeamCreate(team_name: "plan-{feature-slug}")
   ```
...
Task(
  ...
  name: "researcher",
  mode: "plan",
  prompt: "You are a codebase researcher on team plan-{feature-slug}.
...
Task(
  ...
  name: "analyst",
  mode: "plan",
  prompt: "You are a requirements analyst on team plan-{feature-slug}.
...
Task(
  ...
  name: "architect",
  mode: "plan",
  prompt: "You are a technical architect on team plan-{feature-slug}.
...
All three teammates run in parallel. 팀원의 메시지는 오케스트레이터에게 자동으로 전달됨.
...
SPEC output at: .sudapapa/specs/SPEC-XXX/spec.md
