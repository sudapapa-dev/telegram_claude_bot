...
- Triggered by: /review --team OR explicit multi-perspective review request
...
2. Create team:
   ```
   TeamCreate(team_name: "review-{target}")
   ```
...
Task(
  ...
  team_name: "review-{target}",
  name: "security-reviewer",
  mode: "plan",
  prompt: "You are a security reviewer on team review-{target}.
...
Task(
  ...
  name: "perf-reviewer",
  mode: "plan",
  prompt: "You are a performance reviewer on team review-{target}.
...
Task(
  ...
  mode: "plan",
  prompt: "You are a code quality reviewer on team review-{target}.
...
Task(
  ...
  name: "ux-reviewer",
  mode: "plan",
  prompt: "You are a UX reviewer on team review-{target}.
...
All four reviewers run in parallel. 팀원의 메시지는 오케스트레이터에게 자동으로 전달됨.
