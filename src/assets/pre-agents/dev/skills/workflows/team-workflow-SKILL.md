...
name: workflow-team
description: >
  Agent Teams workflow management for 에이전트 시스템. Handles team creation,
...
metadata:
  ...
  related-skills: "workflow-spec, workflow-ddd, workflow-tdd"
...
This skill manages Agent Teams execution for 에이전트 워크플로우s. When team mode is selected (via --team flag, auto-detection, or configuration), 오케스트레이터가 동작 as Team Lead coordinating persistent teammates.
...
```
TeamCreate(team_name: "team-{workflow}-{timestamp}")
```

Team naming convention:
- Plan phase: `plan-SPEC-XXX`
- Run phase: `run-SPEC-XXX`
- Debug: `debug-{issue}`
...
오케스트레이터 (Team Lead) monitors and coordinates:
...
- 오케스트레이터가 동작 in coordination-only mode
