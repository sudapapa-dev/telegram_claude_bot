...
- Approved SPEC document at .sudapapa/specs/SPEC-XXX/
- workflow.team.enabled: true
- CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1
- Triggered by: /run SPEC-XXX --team OR auto-detected complexity >= threshold
...
4. Create team:
   ```
   TeamCreate(team_name: "run-SPEC-XXX")
   ```
...
Task(
  ...
  team_name: "run-SPEC-XXX",
  name: "backend-dev",
  mode: "acceptEdits",
  prompt: "You are the backend developer on team run-SPEC-XXX.
...
Task(
  ...
  name: "frontend-dev",
  mode: "acceptEdits",
  prompt: "You are the frontend developer on team run-SPEC-XXX.
    SPEC Summary: {spec_summary}
...
Task(
  ...
  name: "tester",
  mode: "acceptEdits",
  prompt: "You are the testing specialist on team run-SPEC-XXX.
    SPEC Summary: {spec_summary}
...
Task(
  ...
  name: "designer",
  mode: "acceptEdits",
  prompt: "You are the UI/UX designer on team run-SPEC-XXX.
    SPEC Summary: {spec_summary}
...
```
Task(subagent_type: "team-backend-dev", team_name: "run-SPEC-XXX", name: "api-layer", mode: "acceptEdits", ...)
Task(subagent_type: "team-frontend-dev", team_name: "run-SPEC-XXX", name: "ui-layer", mode: "acceptEdits", ...)
Task(subagent_type: "team-backend-dev", team_name: "run-SPEC-XXX", name: "data-layer", mode: "acceptEdits", ...)
Task(subagent_type: "team-quality", team_name: "run-SPEC-XXX", name: "quality", mode: "plan", ...)
```
...
- 오케스트레이터가 동작 in coordination-only mode during the entire run phase
