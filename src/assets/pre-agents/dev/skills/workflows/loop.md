...
- If session duration exceeds 25 minutes OR iteration time doubling:
  - Save proactive checkpoint to $CLAUDE_PROJECT_DIR/.sudapapa/cache/loop-snapshots/memory-pressure.json
  - Warn user about memory pressure
  - Suggest resuming with /loop --resume memory-pressure
...
- Save iteration snapshot to $CLAUDE_PROJECT_DIR/.sudapapa/cache/loop-snapshots/
...
Snapshot location: $CLAUDE_PROJECT_DIR/.sudapapa/cache/loop-snapshots/
...
Loop state file: $CLAUDE_PROJECT_DIR/.sudapapa/cache/.loop_state.json

Resume commands:
- /loop --resume latest
- /loop --resume iteration-002
- /loop --resume memory-pressure
