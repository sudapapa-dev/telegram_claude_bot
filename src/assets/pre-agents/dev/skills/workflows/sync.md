...
- Implements Step 4 of 4단계 워크플로우 (Report and Commit)
- Receives implementation artifacts from /run
...
- .sudapapa/specs/ directory listing (SPEC documents for sync)
- .sudapapa/project/ directory listing (project documents for conditional update)
- README.md (current project documentation)

Pre-execution commands: git status, git diff, git branch, git log, find .sudapapa/specs.
...
- Create backup directory: .sudapapa/backups/sync-{timestamp}/
- Copy critical files: README.md, docs/, .sudapapa/specs/
...
- Generate sync report: .sudapapa/reports/sync-report-{timestamp}.md
...
Purpose: Update .sudapapa/project/ documents when significant structural changes are detected.
...
Skip condition: If .sudapapa/project/ directory does not exist or contains no files, skip this step entirely.
...
Read `github.git_workflow` from `.sudapapa/config/sections/system.yaml`. This determines how changes are delivered.
...
- Review PR on GitHub
- Auto-Merge PR (/sync --merge)
- Create Next SPEC (/plan)
...
- Display retry command: /sync [mode]
- Exit with code 0
