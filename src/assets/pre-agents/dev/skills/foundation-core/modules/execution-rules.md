...
Purpose: Security policies, execution constraints, and Git workflow strategies governing 에이전트 시스템 agent behavior.
...
오케스트레이터가 동작 under strict execution rules:
...
오케스트레이터가 자동 조정 Git workflow based on `config.json` settings.
...
오케스트레이터 동작 (prompt_always=true):
1. When running `/plan`, user prompted: "Create branch?"
...
오케스트레이터 동작 (prompt_always=false):
...
2. Running `/run`: DDD commits + auto push
3. Running `/sync`: Doc commits + suggest PR creation (user choice)
...
오케스트레이터 동작 (prompt_always=false, auto_enabled=false):
1. When running `/plan`, user prompted once: "Enable automatic branch creation?"
...
오케스트레이터 동작 (prompt_always=false, auto_enabled=true):
...
2. Running `/run`: DDD commits + auto push (to feature branch)
3. Running `/sync`: Doc commits + prepare PR
...
오케스트레이터 동작 (prompt_always=false, auto_enabled=false):
1. When running `/plan`, user prompted once: "Enable automatic branch creation and Draft PR creation?"
...
Execute `/clear` after `/plan` completion in ALL modes.
...
1. Immediately after `/plan` (mandatory)
...
- Create documentation in `.sudapapa/` directory
...
This comprehensive set of execution rules ensures that 에이전트 시스템 operates securely, efficiently, and in compliance with industry standards while maintaining high quality output.
