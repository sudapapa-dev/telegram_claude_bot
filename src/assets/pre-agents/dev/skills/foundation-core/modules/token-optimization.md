...
1. Immediately after /plan (saves 45-50K)
...
def restore_minimal_context(summary: dict):
 ...
 load_file(f".sudapapa/specs/{summary['current_spec']}/spec.md")
...
class SelectiveFileLoader:
 ...
 f".sudapapa/specs/{context['spec_id']}/spec.md"
 ]
 ...
 files = [f".sudapapa/specs/{context['spec_id']}/spec.md"]
...
def load_history():
 load_file(".sudapapa/conversation_history.json") # 500K+ tokens
```
...
Commands:
- /clear - Context reset (mandatory after /plan)
- /context - Check current token usage
- /plan - SPEC generation (30K budget)
- /run - DDD implementation (180K budget)
- /sync - Documentation (40K budget)

Memory:
- Skill("foundation-core") modules/token-optimization.md - Optimization strategies
- @.sudapapa/config/config.json - Budget configuration
