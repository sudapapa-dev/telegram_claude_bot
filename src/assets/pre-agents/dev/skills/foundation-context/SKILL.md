...
metadata:
  ...
  aliases: "foundation-context"
  replaces: "core-context-budget, core-session-state"
...
Mandatory /clear Points: After /plan completion to save 45-50K tokens. When context exceeds 150K tokens to prevent overflow. When conversation exceeds 50 messages to remove stale history. Before major phase transitions for clean slate. During model switches for Haiku to Sonnet handoffs.
...
- cc-memory - Memory management and context persistence
- cc-configuration - Session configuration and preferences
- core-workflow - Workflow state persistence and recovery
- cc-agents - Agent state management across sessions
- foundation-trust - Quality gate integration
...
SPEC-First Workflow: Execute /plan, then mandatory /clear to save 45-50K tokens, then /run SPEC-XXX, then multi-agent handoffs with Pattern 4, then /sync SPEC-XXX, then session state persistence with Pattern 3.
