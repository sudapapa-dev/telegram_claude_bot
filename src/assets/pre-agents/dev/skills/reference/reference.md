...
description: >
  ...
  에이전트 워크플로우s. Provides resume patterns and context propagation guidance.
...
Common patterns, flag reference, legacy command mapping, and configuration files used across all 에이전트 워크플로우s.
...
- 에이전트 워크플로우: Parallel exploration phase, then sequential SPEC generation and DDD implementation
...
- Read existing SPEC document from .sudapapa/specs/SPEC-XXX/
...
- /run --resume SPEC-XXX: Resume full autonomous workflow from last phase
...
Previous /X-Y command format mapped to new  subcommand format:

- /project maps to  project
- /plan maps to /plan
- /run maps to /run
- /sync maps to /sync
- /feedback maps to  feedback
- /fix maps to /fix
- /loop maps to  loop
-  maps to  (default autonomous workflow)

Note: /99-release is a separate local-only command, not part of the  skill.
...
- .sudapapa/config/config.yaml: Main configuration file (merged from section files)
- .sudapapa/config/sections/language.yaml: Language settings (conversation_language, agent_prompt_language, code_comments)
- .sudapapa/config/sections/user.yaml: User identification (name)
- .sudapapa/config/sections/quality.yaml: TRUST 5 framework settings, LSP quality gates, test coverage targets
- .sudapapa/config/sections/system.yaml: System metadata (system.version)

### Project Documentation

- .sudapapa/project/product.md: Product overview, features, user value
- .sudapapa/project/structure.md: Project architecture and directory organization
- .sudapapa/project/tech.md: Technology stack, dependencies, technical decisions

### SPEC Documents

- .sudapapa/specs/SPEC-XXX/spec.md: Specification document with EARS format requirements
- .sudapapa/specs/SPEC-XXX/plan.md: Execution plan with task breakdown
- .sudapapa/specs/SPEC-XXX/acceptance.md: Acceptance criteria and test plan

### Release Artifacts

- CHANGELOG.md: Bilingual changelog (English + Korean per version)
- .sudapapa/cache/release-snapshots/latest.json: Release state snapshot for recovery
...
- .sudapapa/config/config.yaml: Config display version
- .sudapapa/config/sections/system.yaml: System metadata version
...
- `<done>`: Single task or phase completed
- `<complete>`: Full workflow completed (all phases finished)
...
- 에이전트 시스템 errors: Suggest  feedback to create a GitHub issue
