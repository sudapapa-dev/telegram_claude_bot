...
/loop --zero-warnings true
```

**Loop State Management:**

```json
// .sudapapa/cache/.loop_state.json
{
  "active": true,
  "iteration": 1,
  "max_iterations": 10,
...
ralph:
  ...
  ast_grep:
    ...
    quality_scan: true
    custom_rules:
      - .sudapapa/ast-grep/security/**/*.yml
      - .sudapapa/ast-grep/quality/**/*.yml
...
ralph:
  ...
  ast_grep:
    enabled: true
    rules:
      - .sudapapa/ast-grep/react/**/*.yml
      - .sudapapa/ast-grep/typescript/**/*.yml
```
...
def load_loop_state() -> dict:
    state_file = Path(".sudapapa/cache/.loop_state.json")
...
```yaml
# .sudapapa/ast-grep/security/sql-injection.yml
...
```yaml
# .sudapapa/ast-grep/security/xss-prevention.yml
id: xss-react
language: typescript
rule:
...
```yaml
# .sudapapa/ast-grep/security/hardcoded-secrets.yml
...
jobs:
  ralph-check:
    ...
    steps:
      ...
      - name: Check Results
        run: |
          python .sudapapa/scripts/check_ralph_results.py
```
    ...
    steps:
      ...
      - name: Run ${{ matrix.check }} Check
        run: |
          case "${{ matrix.check }}" in
            lsp)
              claude -p "/fix --severity error" \
                --allowedTools "Read,Write,Edit"
              ;;
            ast-grep)
              agent-ast-grep scan --security
              ;;
            tests)
              pytest tests/ -v
              ;;
      ...
      - name: Upload Results
        ...
        with:
          ...
          path: .sudapapa/reports/${{ matrix.check }}
```
...
# Run LSP diagnostics
agent-lsp diagnose --changed-files

if [ $? -ne 0 ]; then
    echo "❌ LSP errors found. Run '/fix' to resolve."
    exit 1
fi

# Run AST-grep security scan
agent-ast-grep scan --security --changed-files
...
# Install 에이전트 시스템
RUN go install github.com/modu-ai-adk/cmd@latest
...
RUN claude -p "/loop --max-iterations 3" \
    --allowedTools "Read,Write,Edit" \
...
```bash
# View LSP logs
cat .sudapapa/logs/lsp_diagnostic.log

# Test LSP connection
agent-lsp test-connection python

# View diagnostics directly
agent-lsp diagnose src/auth.py
```
...
```bash
# View current state
cat .sudapapa/cache/.loop_state.json

# Reset loop state
rm .sudapapa/cache/.loop_state.json

# View loop logs
cat .sudapapa/logs/loop_controller.log
```
...
Check:
  ...
  - AGENT_DISABLE_LOOP_CONTROLLER not set
  - State file writable
