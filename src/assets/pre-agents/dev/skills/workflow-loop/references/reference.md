...
| `ralph.ast_grep.config_path`                        | string  | `.claude/skills/tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
| ------------------------------ | ------- | -------------------------------------- | -------------------------------- |
| `AGENT_DISABLE_LSP_DIAGNOSTIC`  | boolean | Disable LSP diagnostic hook            | `AGENT_DISABLE_LSP_DIAGNOSTIC=1`  |
| `AGENT_DISABLE_LOOP_CONTROLLER` | boolean | Disable loop controller hook           | `AGENT_DISABLE_LOOP_CONTROLLER=1` |
| `AGENT_LOOP_ACTIVE`             | boolean | Loop active flag (set by commands)     | `AGENT_LOOP_ACTIVE=1`             |
| `AGENT_LOOP_ITERATION`          | integer | Current iteration number               | `AGENT_LOOP_ITERATION=3`          |
...
export AGENT_DISABLE_LSP_DIAGNOSTIC=1
claude -p "Make changes"

# Disable loop controller
export AGENT_DISABLE_LOOP_CONTROLLER=1
...
export AGENT_LOOP_ITERATION=0
claude -p "/loop --max-iterations 5"
```
...
> Note: The Go edition provides LSP capabilities through compiled hook subcommands in internal/lsp/. The examples below show the conceptual API; actual invocation is via agent hook post-tool-use.
...
```python
LSPClient(project_root: str | Path)
```
...
Location: `.sudapapa/cache/.loop_state.json`
...
Location: `.claude/skills/tool-ast-grep/rules/sgconfig.yml`
...
1. Check if Ralph is enabled:
   ...
   ```yaml
   # .sudapapa/config/sections/ralph.yaml
   ralph:
     enabled: true
   ```
...
3. Check environment variable:
   ...
   ```bash
   unset AGENT_DISABLE_LOOP_CONTROLLER
   ```

4. Verify state file is writable:
   ```bash
   mkdir -p .sudapapa/cache
   chmod 755 .sudapapa/cache
   ```
...
4. Check environment variable:
   ...
   ```bash
   unset AGENT_DISABLE_LSP_DIAGNOSTIC
   ```
...
3. Delete state file:
   ```bash
   rm .sudapapa/cache/.loop_state.json
   ```
...
3. Check configuration file exists:
   ...
   ```bash
   ls .claude/skills/tool-ast-grep/rules/sgconfig.yml
   ```
    ...
    steps:
      ...
      - name: Commit fixes
        ...
        run: |
          ...
          git config user.email "ralph@agent-system.dev"
          git add .
...
```python
# .claude/hooks/custom_completion_check
